...
function buildEditableOutput(csvData) {
	...
	for(let i = 0; i < fin.headerData.length; i++) {
		...
		txtValues.setAttribute('style', 'font-weight: bold; background: ' + headerTextBoxBackground); 
			...
			//btnDeleteColumn.setAttribute("id", "deleteColumn");
			...
			btnDeleteColumn.onclick = function(e) {
				...
				if (confirm(`Are you sure that you want to delete column?`)) 	
				{
						...
						
						{
								...
								if( parseInt(document.querySelectorAll('th')[b2].querySelector("BUTTON").id.split('_')[1]) == cellNumber)
								{
										...
										for (var j = 1; j < row.length; j++) {
											try
											{
												...
												//alert("ERROR"); 
												//document.getElementsByTagName("th")[cellNumber-1].remove();
												//row[j].deleteCell(cellNumber-1); 
							...
							//if( document.querySelectorAll('th')[b2].querySelector("BUTTON").length > 0  ) {
...
//					let row = tble.rows;
//					document.getElementsByTagName("th")[cellNumber].remove();

					//for (var j = 1; j < row.length; j++) {
//					for (var j = 1; j < row.length; j++) {
//						try
//						{
//							row[j].deleteCell(cellNumber);
//							 
//						}
//						catch(e){
//							alert("ERROR"); 
...
//					}
      ...
      //	th.height = '45';
		th.className = "csvHeader";
      	th.setAttribute('style', 'font-size: 16px; font-weight: bold; background: ' + headerBackground); 
      	//th.appendChild(document.createTextNode("asdf"));
			...
			btnDeleteColumn.onclick = function(e) {
					...
					
				{
						...
						
						{
							...
							//{
								...
								//alert( document.querySelectorAll('th')[b2].querySelector("BUTTON").id);
				...
				for(let b = 1; b < x1.rows.length; b++)
				{
					...
					txtValues.setAttribute('style', 'font-weight: bold; background: #FFFFFF'); 
		...
		tr.onclick = function() { 
			...
			else
			{
				...
				for(let z = 3; z < len1+3; z++){
					...
					try{
						//alert("success");
						//alert(document.getElementById('myTable2').rows[rIndex].cells[z].getElementsByTagName('input')[0].length);
...
function createNewFile() {
	...
	newDivContainer.appendChild(spanContainer2);
 	...
 	btnCreateNewFile.onclick = () => {
		...
		//google.script.run.withSuccessHandler(csvFileCreated).saveCSVFile(newcsvFileName, buildCSVArray);
...
function csvFileCreated() {
	...
	//google.script.run.withSuccessHandler(csvFileLoaded).loadCSVFile2(focusedFileName);  	
...
function csvFileListRetrieved(a) {
	  ...
	  var fileArray = [];
	      ...
	      if(index == 0) {
			...
			for(var i = 0; i < element.length; i++) { headerArray.push(element[i]); }
			...
			let fileEntry = { name: element[0], fileSize: element[1],
						fileType: element[2], fileCreationDate: element[3]};
...
function buildReadOnlyOutput(csvData) {
	...
	var tableBody = document.createElement('TBODY');
  	...
  	for (var i = 0; i < fin.fileData.length; i++) {
		...
		tr.onmouseover = function() { this.style.backgroundColor = "#D4F1F4"; };
	...
	setTimeout(() => {
			...
			switch(clickEventType) {
				case 0:
					...
					document.querySelector('#content_container').innerHTML = "Loading CSV File, please wait...";
					//google.script.run.withSuccessHandler(csvFileLoaded).loadCSVFile2(theCSVFile);  
				...
				case 2:  
					if (confirm(`Are you sure that you want to delete file ${focusedFileName}?`)) {
						...
						//google.script.run.withSuccessHandler(deleteCSVFileSuccess).deleteCSVFile(focusedFileName); 					 
...
//			{
//				if(tr[j].onmouseover != null) {
					
					//alert("work??");
//					tr[i].onmouseover = tr[j].onmouseover;
//					tr[i].onmouseout = tr[j].onmouseout;
//					tr[i].onclick = tr[j].onclick;
					//break;
//				}
//			}
...
function reassignBtnDeleteColumnIDs() {
	...
	for(let j = 0; j <	document.querySelectorAll('.deleteColumn').length; j++) {
			...
			//alert(document.querySelectorAll('.deleteColumn')[j].id);
	...
	for(var i = 0; i < rLength-1; i++ ) {
		...
		a = lastRowClickedIndex-1;
			...
			for(var b = 0; b < rLength-1; b++) {
				...
				else {  document.querySelectorAll(".classEditFileButtons")[b].style.backgroundColor = btnBackgroundColor ; }
			}
